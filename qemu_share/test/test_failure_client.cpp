...
void test_failure(DiancieClient<TestFailFunctions> &client) {
	auto a = client.shm_new<int>(2);
	client.call<TestFailFunctions::ADD>(a);
	std::cout << *a << std::endl;
	assert(*a == 4);
}

void test_failure_two(DiancieClient<TestFailFunctions> &client) {
	auto p = client.shm_new(Person{5, 6});
	client.call<TestFailFunctions::PERSON>(p);
	assert(p->age == 6);
	assert(p->income == 7);
}
...
int main(int argc, char *argv[]) {
	try {
		...
		// Create client
		DiancieClient<TestFailFunctions> client(device_path, service_name,
		                                        instance_id);

		std::cout << "Client connected successfully!" << std::endl;

		// Run test suite
		test_failure_two(client);
